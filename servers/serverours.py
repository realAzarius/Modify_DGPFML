...
class ServerOurs(Server):
    ...
    def __init__(self, dataset, algorithm, model, k_meta_models, num_k, num_select_clients, batch_size, inner_lr,
                 outer_lr, local_epochs, test_epochs, num_round, eval_gap):
        super(ServerOurs, self).__init__(dataset, algorithm,
                                         model[0], num_select_clients, batch_size, inner_lr, outer_lr, local_epochs,
                                         num_round)
        ...
        self.cluster_centers, _ = self.pre_cluster(self.dataset)
        self.cluster_centers = list(self.cluster_centers)
    ...
    def save_results(self):
        ...
        if (len(self.rs_glob_acc) != 0 and len(self.rs_train_acc) and len(self.rs_train_loss)):
            with h5py.File(f"./results/DGFML/{mon}+'-'+{day}+'-'+{hour}+'-'+{minute}+'-'" + '{}.h5'.format(alg), 'w') as hf:
    ...
    def pre_cluster(self, dataset):
        ...
        if self.model_name == "lstm":
            ...
            global_ae = Autoencoder(input_size=3072, embedding_size=64)
        # print(global_ae)  # out_features 784 3072
        # print(self.global_average_ae)
        ...
        # eps_values = [1, 2]  # eps 值，控制点的邻域范围
        # min_samples_values = [5, 7]
        # eps_values = [i for i in [0.01, 0.1, 1, 2] if 0.01 <= i <= 2]
        # min_samples_values = [i for i in [2, 3, 4, 5, 6, 7] if
        #                       2 <= i <= 7]

        eps_values = [1, 2]  # 根据 k 距离图选择范围
        min_samples_values = [5, 7]  # 根据降维后的维度调整范围
        best_k = -1
